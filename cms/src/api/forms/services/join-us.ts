 ...
 * join-us service
 */
const OTP_TYPE = 'join_us';
interface JoinUsFormRequest {
    first_name: string;
    last_name: string;
    mobile: string;
    email: string;
...
export default {
    saveContent: async (req: JoinUsFormRequest): Promise<FormResponse> => {
        try {
            ...
            if (validation.status === HTTP_STATUS_SUCCESS) {
                await strapi.documents('api::join-us.join-us').create({
                    data: {
